...
def write_text_files(project_fd, installer_fd, game_path):
    for path in project_fd.iterdir():
        ...
        if "LICENSE" in path.name or "README" in path.name:
            installer_fd.write(delete_file(f"/home/guest/Sources/{project_path}/{path.name}"))
            installer_fd.write(create_file(f"/home/guest/Sources/{project_path}/{path.name}"))
            with open(path.absolute(), "r") as path_fd:
                ...
                for line in lines:
                    installer_fd.write(append(f"/home/guest/Sources/{project_path}/{path.name}", line))
...
if __name__ == "__main__":
    ...
    with open(f"scripts/{filename}", "w") as installer:
        ...
        installer.write(create_folder(f"/home/guest/Sources/{project_path}"))

        root = Path(".")
        create_project_structure_recursively(root, installer, f"/home/guest/Sources/{project_path}")
        write_text_files(root, installer, f"/home/guest/Sources/{project_path}")
